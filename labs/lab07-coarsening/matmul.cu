...
__global__ void mm_tiled_coarse_kernel(float* A, float* B, float* C, unsigned int N) {
    ...
    // TODO: 슬라이드 22를 옮긴다.
    //
    //   (1) row 와 colStart 를 구한다
    //         row      = blockIdx.y*blockDim.y + threadIdx.y
    //         colStart = blockIdx.x*blockDim.x*COARSE_FACTOR + threadIdx.x
    //
    //   (2) float sum[COARSE_FACTOR]; 를 두고, c 를 0부터 COARSE_FACTOR 전까지
    //       돌며 전부 0.0f 로 채운다
    //
    //   (3) tile 을 0부터 N/TILE_DIM 전까지 돌며
    //         - A 타일 한 칸을 A_s[threadIdx.y][threadIdx.x] 에 적재한다
    //             읽을 위치는 A[row*N + tile*TILE_DIM + threadIdx.x]
    //           A 타일은 c 루프 바깥에서 한 번만 적재한다. 이것이 coarsening의 핵심이다
    //         - c 를 0부터 COARSE_FACTOR 전까지 돌며
    //             col = colStart + c*TILE_DIM
    //             B 타일 한 칸을 B_s[threadIdx.y][threadIdx.x] 에 적재한다
    //               읽을 위치는 B[(tile*TILE_DIM + threadIdx.y)*N + col]
    //             블록 안 모든 스레드가 적재를 끝낼 때까지 기다린다
    //             i 를 0부터 TILE_DIM 전까지 돌며
    //               sum[c] += A_s[threadIdx.y][i]*B_s[i][threadIdx.x];
    //             다음 c 가 B_s 를 덮어쓰기 전에 다시 기다린다
    //
    //   (4) c 를 0부터 COARSE_FACTOR 전까지 돌며
    //         col = colStart + c*TILE_DIM
    //         C[row*N + col] = sum[c];
