    ...
    if (outRow < height && outCol < width) {
        ...
        for(int inRow = outRow - BLUR_SIZE; inRow < outRow + BLUR_SIZE + 1; ++inRow) {
            for(int inCol = outCol - BLUR_SIZE; inCol < outCol + BLUR_SIZE + 1; ++inCol) {
                ...
                // TODO: 읽으려는 (inRow, inCol)이 이미지 안에 있을 때만
                //       average에 image의 그 픽셀을 더하고, count를 1 늘린다.
                //
                //       이미지 안이라는 조건은 네 가지다:
                //         inRow가 0 이상,  inRow가 height 미만,
                //         inCol이 0 이상,  inCol이 width 미만
                //
                //       읽을 위치는 image[inRow*width + inCol] 이다.
