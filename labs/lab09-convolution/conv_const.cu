// lab09 — 컨볼루션 (필터를 상수 메모리에 둔 버전)
// 이 파일은 완성 상태다. 고칠 곳이 없다.
#include <stdio.h>
#include <stdlib.h>
#include "common.cuh"
#include "pgm.h"
...
// ---------------------------------------------------------------------------
// 필터를 상수 메모리에 둔다. 모든 스레드가 같은 값을 읽으므로
// 브로드캐스트가 되고, 전역 메모리 접근에서 빠진다.
// ---------------------------------------------------------------------------
__constant__ float filter_c[FILTER_DIM][FILTER_DIM];

// ---------------------------------------------------------------------------
// 슬라이드 11 — Convolution Kernel Code
// ---------------------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    // 필터를 상수 메모리로 올린다.
    CUDA_CHECK(cudaMemcpyToSymbol(filter_c, filter,
                                  (size_t)FILTER_DIM*FILTER_DIM*sizeof(float)));
    ...
    // 한 번 예열한 뒤 측정한다
    convolution_kernel<<<grid, block>>>(input_d, output_d, width, height);
    ...
    // 그대로 센다. 탭마다 입력 4바이트를 읽으므로 2*D^2 FLOP / (4*D^2 + 4) 바이트다.
    // 필터를 상수 메모리에 두었으므로 필터 읽기는 전역 접근에서 빠진다.
    double slide20 = (2.0*FILTER_DIM*FILTER_DIM)/(4.0*FILTER_DIM*FILTER_DIM + 4.0);

    printf("커널        : %8.2f ms\n", ms);
    printf("유효 대역폭 : %8.1f GB/s\n", traffic_bytes/sec/1e9);
    printf("연산 처리율 : %8.1f GFLOP/s\n", flop/sec/1e9);
    printf("연산/바이트 : %8.1f FLOP/B  (최소 트래픽 기준)\n", flop/traffic_bytes);
    printf("슬라이드 20 : %8.2f FLOP/B  (커널이 요청하는 전역 접근 기준)\n", slide20);
    printf("정확성: %s\n", ok ? "PASS" : "FAIL");
