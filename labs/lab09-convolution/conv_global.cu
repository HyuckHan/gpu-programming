        ...
        for(int filterRow = 0; filterRow < FILTER_DIM; ++filterRow) {
            for(int filterCol = 0; filterCol < FILTER_DIM; ++filterCol) {
                ...
                if(inRow >= 0 && inRow < height && inCol >= 0 && inCol < width) {
                    // TODO: 아래 1.0f 자리에 필터 값이 들어가야 한다.
                    //       상수 메모리 버전은 2차원 배열에서 [filterRow][filterCol] 로 읽었다.
                    //       여기서는 매개변수로 받은 전역 메모리 배열 filter 에서 읽는다.
                    //       filter 는 1차원이다. 행 길이가 FILTER_DIM 이라고 보고
                    //       filterRow 와 filterCol 을 하나의 인덱스로 합쳐라.
                    sum += 1.0f*input[inRow*width + inCol];
                }
            }
        }
