...
__global__ void stencil_register_kernel(float* in, float* out, unsigned int N) {
    ...
    // TODO 1: inPrev 를 공유 메모리가 아닌 레지스터(float)로 선언한다.
    //         아래 한 줄을  float inPrev;  로 바꾼다.
    __shared__ float inPrev_s[IN_TILE_DIM][IN_TILE_DIM];   // ← 이 줄을 고친다
    __shared__ float inCurr_s[IN_TILE_DIM][IN_TILE_DIM];   // 이 줄은 그대로 둔다
    // TODO 2: inNext 도 같은 방식으로 레지스터로 선언한다.
    __shared__ float inNext_s[IN_TILE_DIM][IN_TILE_DIM];   // ← 이 줄을 고친다

    // TODO 1, 2 를 고치면 아래 적재도 inPrev / inNext 로 바뀐다.
    if(iStart - 1 >= 0 && iStart - 1 < N && j >= 0 && j < N && k >= 0 && k < N) {
        inPrev_s[threadIdx.y][threadIdx.x] = in[(iStart - 1)*N*N + j*N + k];
    }
    if(iStart >= 0 && iStart < N && j >= 0 && j < N && k >= 0 && k < N) {
    ...
    for(int i = iStart; i < iStart + OUT_TILE_DIM; ++i) {
            ...
            if(threadIdx.y >= 1 && threadIdx.y < IN_TILE_DIM - 1
               && threadIdx.x >= 1 && threadIdx.x < IN_TILE_DIM - 1) {
                out[i*N*N + j*N + k] = C0*inCurr_s[threadIdx.y][threadIdx.x]
                                     ...
                                     + C3*inCurr_s[threadIdx.y - 1][threadIdx.x]
                                     + C4*inCurr_s[threadIdx.y + 1][threadIdx.x]
                                     // TODO 3: C5, C6 항을 레지스터에서 읽도록 바꾼다.
                                     //         첨자가 없어지고 이름만 남는다.
                                     + C5*inPrev_s[threadIdx.y][threadIdx.x]
                                     + C6*inNext_s[threadIdx.y][threadIdx.x];
            }
        ...
        __syncthreads();
        // TODO 4: 슬라이스 교대를 레지스터 기준으로 바꾼다.
        //         공유 메모리 자리에서 읽어 레지스터에 넣고,
        //         레지스터 값을 공유 메모리 자리에 넣는 모양이 된다.
        inPrev_s[threadIdx.y][threadIdx.x] = inCurr_s[threadIdx.y][threadIdx.x];
        inCurr_s[threadIdx.y][threadIdx.x] = inNext_s[threadIdx.y][threadIdx.x];
    }
