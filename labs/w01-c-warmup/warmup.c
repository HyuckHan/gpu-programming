...
void mat_mul(const float* A, const float* B, float* C, int N) {
    ...
    // TODO: 삼중 루프로 채운다.
    //       바깥 두 루프는 row와 col을 훑는다 (mat_add와 같다).
    //       가장 안쪽 루프에서 i를 0부터 N 전까지 돌며
    //         sum += A[row*N + i] * B[i*N + col];
    //       를 누적하고, 다 더한 sum을 C[row*N + col]에 넣는다.
    //
    //       A는 row행을 가로로 훑고, B는 col열을 세로로 훑는다.
    //       B의 첨자가 i*N + col 인 이유를 종이에 그려 보면 확실해진다.
...
void mat_transpose(const float* A, float* B, int rows, int cols) {
    ...
    // TODO: 이중 루프로 채운다.
    //       row는 0부터 rows 전까지, col은 0부터 cols 전까지 돌며
    //         B[col*rows + row] = A[row*cols + col];
    //
    //       A의 한 줄 길이는 cols이고 B의 한 줄 길이는 rows다.
    //       둘을 바꿔 쓰면 정사각 행렬에서는 통과하지만 그렇지 않으면 무너진다.
