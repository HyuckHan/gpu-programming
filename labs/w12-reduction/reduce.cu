...
__global__ void reduce_shared_kernel(float* input, float* sum, unsigned int N) {
    ...
    //   (2) input_s[threadIdx.x] 에 input[i] 와 input[i + BLOCK_DIM] 의 합을 넣는다
    //   (3) 블록 안 모든 스레드가 적재를 끝낼 때까지 기다린다
    //   (4) stride 를 BLOCK_DIM/2 부터 1까지 절반씩 줄이며
    //       threadIdx.x < stride 인 스레드가
...
__global__ void reduce_coarse_kernel(float* input, float* sum, unsigned int N) {
    ...
    // TODO: 슬라이드 20을 옮긴다. 4단계와 다른 곳은 앞부분뿐이다.
    //   (1) segment = COARSE_FACTOR*2*blockDim.x*blockIdx.x
    //       i       = segment + threadIdx.x
    //   (2) threadSum 을 0.0f 로 두고, c 를 0부터 COARSE_FACTOR*2 전까지 돌며
    //         threadSum += input[i + c*BLOCK_DIM];
    //   (3) input_s[threadIdx.x] 에 threadSum 을 넣고 기다린다
    //   (4) 그 뒤 리덕션 트리는 4단계와 완전히 같다
