...
__global__ void histogram_private_kernel(unsigned char* image, unsigned int* bins,
    ...
    // 사본 초기화. NUM_BINS 가 blockDim.x 보다 클 수도 있으므로 스트라이드로 돈다.
    for(unsigned int bin = threadIdx.x; bin < NUM_BINS; bin += blockDim.x) {
        bins_s[bin] = 0;
    }
    __syncthreads();

    // 세기. 경합이 블록 안으로 좁아지고, 공유 메모리 atomic 은 전역보다 훨씬 싸다.
    unsigned int i = blockIdx.x*blockDim.x + threadIdx.x;
    if(i < width*height) {
        unsigned char b = image[i];
    ...
    // 사본을 전역에 합친다. 전역 atomic 은 블록당 NUM_BINS 번으로 줄어든다.
    // 0인 칸은 건너뛴다. 더해 봐야 값이 그대로다.
    for(unsigned int bin = threadIdx.x; bin < NUM_BINS; bin += blockDim.x) {
